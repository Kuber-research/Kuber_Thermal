...
\IEEEauthorblockA{\texttt{github.com/Kuber-research/Kuber_Thermal}\\
...
cross-attention adds a learned wall-normal penalty - \emph{anisotropic boundary-layer} (ABL) attention - that biases each query to
...
introduced physics attention over learned slices; Universal Physics Transformers (UPT)~\cite{upt} and the anchored/branched
...
\subsection{KuberNet}\label{sec:kubernet}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{architecture_updated.png}
...
extra noised-field input channel. With a geometric noise schedule $\sigma_k=\sigma_{\min}^{\,k/K}$ for $k\ge 1$
($\sigma_0=0$, the clean step), each training sample draws $k\sim\mathrm{Uniform}\{0,\dots,K\}$: at $k{=}0$ the network
...
mean-squared-error fit over-smooths. The out-of-distribution accuracy in Sec.~VI is reported from the
...
We generate CHT data with OpenFOAM's steady buoyant solver (\texttt{buoyantSimpleFoam})~\cite{weller} over a single
...
the medium out-of-distribution split, beating the previously published best (UPT, 12.41\,K) with no domain
adaptation. The gain is driven by its \emph{anisotropic boundary-layer} attention (Sec.~\ref{sec:kubernet}):
ablating it to isotropic kNN raises error to 12.14\,K / 0.671 - a 10\% larger mean nRMSE at equal parameter
...
KuberNet's anisotropic boundary-layer attention (Sec.~\ref{sec:kubernet}) drives the SIMSHIFT result, but
the surface-conditioned backbone it builds on is itself geometry-general: a single set of weights,
distinguished only by a device flag and fluid/BC conditioning, spans a wall-temperature, buoyancy-driven
heatsink in air and a heat-flux, forced-liquid cold plate. One multi-geometry model - trained on our corpus
with full geometry and physics conditioning - reaches the per-class errors in Table~\ref{tab:multi} when
evaluated on held-out cases (there is no public cold-plate CHT benchmark); the cold-plate mean nRMSE (0.028)
is close to the in-distribution value. These numbers are on our own corpus and are not comparable to the
SIMSHIFT numbers, and this multi-geometry checkpoint predates the ABL refinement - folding ABL into it
end-to-end is a direct extension we are pursuing.
...
\subsection{Pretraining and Reduced Conditioning}\label{sec:pretrain}
The headline results (Tables~\ref{tab:lead}-\ref{tab:srctgt}) condition on the same 12
...
pretraining help beyond the benchmark's own training set? We run a controlled A/B with the same model and SIMSHIFT
...
slightly conservatively, the safe direction.
...
error, which we report transparently. Inference latency is an estimate. The multi-geometry numbers are on our
own corpus and are not comparable to the public benchmark.

\textit{Future work.} ABL attention (Sec.~\ref{sec:kubernet}) and Buckingham-$\Pi$ conditioning with corpus
pretraining (Sec.~\ref{sec:pretrain}) are realized in the present model; the directions below remain, each
